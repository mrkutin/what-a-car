...
try {
    await redisSub.xgroup('CREATE', 'stream:captcha:diagnostic-cards:resolved', 'gibdd:diagnostic-cards', '$', 'MKSTREAM')
} catch (e) {
    console.log('Group "gibdd:diagnostic-cards" already exists in stream:captcha:diagnostic-cards:resolved, skipping')
}
...
async function listenForMessages(/*lastId = '$'*/) {
    ...
    const captchaVinResults = await redisSub.xreadgroup('GROUP', 'gibdd:diagnostic-cards', hostId, 'BLOCK', HEARTBEAT_INTERVAL_MS, 'COUNT', 1, 'STREAMS', 'stream:captcha:diagnostic-cards:resolved', '>')
        ...
        for (const message of flatMessagesWithVin) {
            ...
            if (vin) {
                ...
                if (value && chatSettings?.cache === true) {
                    //debounce
                    const history = await redisPub.xrevrange('stream:gibdd:diagnostic-cards:resolved', '+', Date.now() - DEBOUNCE_INTERVAL_MS, 'COUNT', DEBOUNCE_COUNT)
                    ...
                    if (idx === -1) {
                        await redisPub.xadd('stream:gibdd:diagnostic-cards:resolved', '*', 'key', key, 'chat_id', chat_id, 'plate', plate, 'vin', vin)
                    }
                } else {
                    //debounce
                    const history = await redisPub.xrevrange('stream:captcha:diagnostic-cards:requested', '+', Date.now() - DEBOUNCE_INTERVAL_MS, 'COUNT', DEBOUNCE_COUNT)
                    const idx = history.findIndex(message => {
                    ...
                    if (idx === -1) {
                        ...
                        await redisPub.xadd('stream:captcha:diagnostic-cards:requested', '*', 'key', captchaKey, 'token', captchaToken, 'vin', vin, 'chat_id', chat_id, 'plate', plate)
                    }
                }
            }
        }
