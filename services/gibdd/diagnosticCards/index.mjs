...
try {
    await redisSub.xgroup('CREATE', 'stream:captcha:resolved', 'gibdd:diagnostic-cards', '$', 'MKSTREAM')
} catch (e) {
    console.log('Group "gibdd:diagnostic-cards" already exists in stream:captcha:resolved, skipping')
}
...
async function listenForMessages(/*lastId = '$'*/) {
    ...
    for (const message of flatCaptchaMessages) {
        ...
        const diagnosticCards = await getDiagnosticCardsByVin({captchaToken, captchaWord, vin} )

        const key = `gibdd:diagnostic-cards:${vin}`
        await redisPub.call('JSON.SET', key, '$', JSON.stringify(diagnosticCards))
