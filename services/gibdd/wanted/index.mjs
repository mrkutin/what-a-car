...
try {
    await redisSub.xgroup('CREATE', 'stream:vin:resolved', 'gibdd:wanted', '$', 'MKSTREAM')
} catch (e) {
    console.log('Group "gibdd:wanted" already exists in stream:vin:resolved, skipping')
}
//captcha needs to be processed by one host of one service, so the group name is common
try {
    await redisSub.xgroup('CREATE', 'stream:captcha:wanted:resolved', 'gibdd:wanted', '$', 'MKSTREAM')
} catch (e) {
    console.log('Group "gibdd:wanted" already exists in stream:captcha:wanted:resolved, skipping')
}

const hostId = makeId(7)

async function listenForMessages(/*lastId = '$'*/) {
    await redisPub.set(`heartbeat:gibdd:wanted:${hostId}`, 1, 'PX', 2 * HEARTBEAT_INTERVAL_MS)

    const vinResults = await redisSub.xreadgroup('GROUP', 'gibdd:wanted', hostId, 'BLOCK', HEARTBEAT_INTERVAL_MS, 'COUNT', 1, 'STREAMS', 'stream:vin:resolved', '>')
    const captchaVinResults = await redisSub.xreadgroup('GROUP', 'gibdd:wanted', hostId, 'BLOCK', HEARTBEAT_INTERVAL_MS, 'COUNT', 1, 'STREAMS', 'stream:captcha:wanted:resolved', '>')
        ...
        for (const message of flatMessagesWithVin) {
            ...
            if (vin) {
                ...
                if (value && chatSettings?.cache === true) {
                    ...
                    if (idx === -1) {
                        await redisPub.xadd('stream:gibdd:wanted:resolved', '*', 'key', key, 'chat_id', chat_id, 'plate', plate, 'vin', vin)
                    }
                } else {
                    //debounce
                    const history = await redisPub.xrevrange('stream:captcha:wanted:requested', '+', Date.now() - DEBOUNCE_INTERVAL_MS, 'COUNT', DEBOUNCE_COUNT)
                    const idx = history.findIndex(message => {
                    ...
                    if (idx === -1) {
                        ...
                        await redisPub.xadd('stream:captcha:wanted:requested', '*', 'key', captchaKey, 'token', captchaToken, 'vin', vin, 'chat_id', chat_id, 'plate', plate)
                    }
                }
            }
        }
    ...
    if (captchaVinResults?.length) {
        ...
        for (const message of flatCaptchaMessages) {
            ...
            const key = `gibdd:wanted:${vin}`
            const res = await getWantedByVin({captchaToken, captchaWord, vin})
