...
try {
    await redisSub.xgroup('CREATE', 'stream:captcha:resolved', 'gibdd:wanted', '$', 'MKSTREAM')
} catch (e) {
    console.log('Group "gibdd:wanted" already exists in stream:captcha:resolved, skipping')
}
...
async function listenForMessages(/*lastId = '$'*/) {
    ...
    for (const message of flatCaptchaMessages) {
        ...
        const wanted = await getWantedByVin({captchaToken, captchaWord, vin} )

        const key = `gibdd:wanted:${vin}`
        await redisPub.call('JSON.SET', key, '$', JSON.stringify(wanted))
