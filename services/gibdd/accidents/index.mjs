...
try {
    await redisSub.xgroup('CREATE', 'stream:captcha:accidents:resolved', 'gibdd:accidents', '$', 'MKSTREAM')
} catch (e) {
    console.log('Group "gibdd:accidents" already exists in stream:captcha:accidents:resolved, skipping')
}
...
async function listenForMessages(/*lastId = '$'*/) {
    ...
    const vinResults = await redisSub.xreadgroup('GROUP', 'gibdd:accidents', hostId, 'BLOCK', HEARTBEAT_INTERVAL_MS, 'COUNT', 1, 'STREAMS', 'stream:vin:resolved', '>')
    const captchaVinResults = await redisSub.xreadgroup('GROUP', 'gibdd:accidents', hostId, 'BLOCK', HEARTBEAT_INTERVAL_MS, 'COUNT', 1, 'STREAMS', 'stream:captcha:accidents:resolved', '>')
        ...
        for (const message of flatMessagesWithVin) {
            ...
            if (vin) {
                ...
                if (value && chatSettings?.cache === true) {
                    //debounce
                    const history = await redisPub.xrevrange('stream:gibdd:accidents:resolved', '+', Date.now() - DEBOUNCE_INTERVAL_MS, 'COUNT', DEBOUNCE_COUNT)
                    ...
                    //debounce
                    const history = await redisPub.xrevrange('stream:captcha:accidents:requested', '+', Date.now() - DEBOUNCE_INTERVAL_MS, 'COUNT', DEBOUNCE_COUNT)
                    const idx = history.findIndex(message => {
                    ...
                    if (idx === -1) {
                        ...
                        await redisPub.xadd('stream:captcha:accidents:requested', '*', 'key', captchaKey, 'token', captchaToken, 'vin', vin, 'chat_id', chat_id, 'plate', plate)
                    }
                }
            }
        }
